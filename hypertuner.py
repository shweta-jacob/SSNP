...
class HyperTuningSearchSpace:
    lr = [0.0005, 0.001, 0.005, 0.01, 0.1]
    hidden_dim = [8, 16, 32, 64, 128]
    conv_layer = [1, 2, 3]


permutations = list(itertools.product(
    HyperTuningSearchSpace.lr, HyperTuningSearchSpace.hidden_dim, HyperTuningSearchSpace.conv_layer
))
...
for perm in perms:
    params.update({'lr': perm[0]})
    params.update({'hidden_dim': perm[1]})
    params.update({'conv_layer': perm[2]})
    print(f"Combination lr:{perm[0]}, hidden_dim:{perm[1]}, conv_layer:{perm[2]}")
