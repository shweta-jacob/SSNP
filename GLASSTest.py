...
def split():
    ...
    print("Creating Plain GNN dataset")

    trn_dataset = SubGDataset.GDataset(*train)
    val_dataset = SubGDataset.GDataset(*valid)
    tst_dataset = SubGDataset.GDataset(*test)

    print("Creating GNN seg dataset")
...
def buildModel(hidden_dim, conv_layer, dropout, jk, pool1, pool2, z_ratio, aggr):
    ...
    if args.use_deg:
        print("load ", f"./Emb/{args.dataset}_{input_channels}.pt")
        emb = torch.load(f"./Emb/{args.dataset}_{input_channels}.pt",
